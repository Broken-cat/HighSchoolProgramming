...
class SimpleTree<T> {
	...
	public void recNodeAdd(SimpleTreeNode<T> _startNode, SimpleTreeNode<T> _parent, SimpleTreeNode<T> _nodeToAdd) {
		if (_startNode.equals(_parent)) {
			...
			return;
		} else if (_startNode.Children != null)
			recListAdd(_startNode.Children, _parent, _nodeToAdd);
	...
	public void DeleteNode(SimpleTreeNode<T> NodeToDelete) {
		if (NodeToDelete.NodeValue.equals(Root))
			return;
	...
	public void recListFindBy(SimpleTreeNode<T> _startNode, List<SimpleTreeNode<T>> list, T val,
			List<SimpleTreeNode<T>> _listToAdd) {
		for (int i = 0; i < list.size(); i++)
	...
	public void recNCount(SimpleTreeNode<T> _startNode) {
		if (_startNode.Children == null || _startNode.Children.size() == 0)
			Leaf++;
		else
	...
	public void recNodeForest(SimpleTreeNode<T> _startNode, int num) {
		...
		if (_startNode.Children != null)
			recListForest(_startNode.Children, num);
	}
